        ...
        return (<div key={it.id} className="flex flex-col gap-2">
          ...
          <div className="flex items-center justify-between gap-4 pl-6">
            <div className="flex items-center gap-2">
              ...
              <RestTimer key={`${keys[`set:${key}`]||0}`} seconds={defaultSet} autostart={autoStart===`set:${key}`} label="Rust tussen sets" onDone={()=> setAutoStart('')} />
            </div>
            <div className="flex items-center gap-2">
              ...
              <RestTimer key={`ex-${keys[`ex:${key}`]||0}`} seconds={defaultEx} autostart={autoStart===`ex:${key}`} label="Rust tussen oefeningen" onDone={()=> setAutoStart('')} />
            </div>
          </div></div>)})}
      </div>
